...
xi_to_xj_interactions <- function (initial_inf_stat_vec, inf_prob_vec, interaction_matrix) {
  ...
  for (xi_row in 1:nrow(interaction_matrix)) { # Iterate through each row item of the interaction matrix for person xi
    ...
    xixj_interactions <- interaction_matrix[xi_row,] # Vector of which xj persons that xi has interacted with
...
initial_inf_stat_vec <- initial_inf_stat_vec(20, 4)
...
test_iterate_interactions <- function(initial_inf_stat_vec, inf_prob_vec, interaction_matrix, num) {
  ...
  total_inf_vec[[1]] <- sum(updated_inf_stat_vec[[1]]) # first total # of infected item
  ...
  for (i in 2:(num+1)) {
    ...
    total_inf_vec <- c(total_inf_vec, sum(updated_inf_stat_vec[[i - 1]]))
